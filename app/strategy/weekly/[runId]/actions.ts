...
export type WeeklyActionState = { error: string | null; success?: boolean; stageNumber?: number };
export async function generateWeekAction(_state: WeeklyActionState, formData: FormData): Promise<WeeklyActionState> {
  ...
  revalidatePath(`/strategy/weekly/${parsed.data.runId}`);
  return { error: null, success: true, stageNumber: parsed.data.stageNumber };
}
