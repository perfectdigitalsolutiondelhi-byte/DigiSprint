...
export const marketingStrategyCalendarPrompt: PromptDefinition<MarketingStrategyCalendarInput, MarketingStrategyCalendar> = {
  ...
  render: (input, context) => ["<trusted_business_context>", JSON.stringify({ name: context.name, industry: context.industry, location: context.location }), "</trusted_business_context>", "<executive_strategy_context>", JSON.stringify(input.executiveContext), "</executive_strategy_context>", "<strategy_request>", JSON.stringify(input.strategyRequest), "</strategy_request>", "Create the complete 30-day calendar. Days must be unique and sequential from 1 through 30."].join("\n"),
};
