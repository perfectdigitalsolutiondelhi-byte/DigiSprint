...
export const marketingStrategyExecutionPrompt: PromptDefinition<MarketingStrategyExecutionInput, MarketingStrategyExecution> = {
  ...
  render: (input, context) => ["<trusted_business_context>", JSON.stringify(context), "</trusted_business_context>", "<strategy_foundation>", JSON.stringify(input.foundation), "</strategy_foundation>", "<strategy_request>", JSON.stringify(input.strategyRequest), "</strategy_request>", "Build the complete execution plan that will guide a separate 30-day calendar."].join("\n"),
};