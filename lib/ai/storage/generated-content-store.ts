...
export async function completeAIJob<T>(supabase: SupabaseClient, values: { jobId: string; featureKey: string; promptKey: string; promptVersion: number; language: string; output: T; result: AIProviderResult<T>; durationMs: number }) {
  ...
  const { data, error } = await supabase.rpc("complete_ai_job", {
    ...
    target_language: values.language, target_content: values.output,
