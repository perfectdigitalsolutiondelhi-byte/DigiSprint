...
export async function loadAISettings(supabase: SupabaseClient, businessId: string) {
  const { data, error } = await supabase.from("ai_settings").select("is_enabled,model_profile,default_language,max_output_tokens,include_business_context").eq("business_id", businessId).maybeSingle();
