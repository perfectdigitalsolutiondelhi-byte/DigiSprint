...
const previewWorkspace: DashboardWorkspace = {
  configured: false,
  userName: "DigiSprint owner",
  userEmail: null,
  business: { id: "preview", name: "Your business", industry: "Small business", city: "India", description: "Your business context will appear here after setup." },
  brand: { tone: "Friendly", primary_color: "#6366F1", language_preferences: ["en"] },
  preferences: { platforms: ["instagram"], content_goals: ["visibility"], target_audience: "Your ideal local customers", posts_per_week: 3 },
  recentPosts: [],
  postCount: 0,
  readyCount: 0,
};
...
export async function loadDashboard(): Promise<DashboardWorkspace> {
  ...
  const [businessResult, brandResult, preferencesResult, postsResult, countResult, readyResult] = await Promise.all([
