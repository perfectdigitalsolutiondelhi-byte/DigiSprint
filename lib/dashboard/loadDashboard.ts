...
export async function loadDashboard(): Promise<DashboardWorkspace> {
  ...
  const supabase=await createClient();const {data:{user},error:authError}=await supabase.auth.getUser();if(authError)throw new AIPlatformError("STORAGE_ERROR","The signed-in user could not be verified.",authError);if(!user)redirect("/login?next=/dashboard");
  const {
  data: membership,
  error: membershipError,
} = await supabase
.from("business_members").select("business_id").eq("user_id",user.id).eq("is_active",true).limit(1).maybeSingle();if(membershipError)throw new AIPlatformError("STORAGE_ERROR","The dashboard workspace could not be loaded.",membershipError);if(!membership)redirect("/setup");
  ...
  const queryError=businessResult.error||brandResult.error||preferencesResult.error||postsResult.error||countResult.error||readyResult.error||strategyResult.error||strategyCount.error;
  if(queryError)throw new AIPlatformError("STORAGE_ERROR","Unable to load the dashboard workspace.",queryError);
  if(!businessResult.data)throw new AIPlatformError("BUSINESS_NOT_FOUND","Unable to load the business workspace.");
